...
void GameApp::OnInitialize(uint32_t width, uint32_t height)
{
	...
	//Changed
	mBallTexture.Initialize(L"../Textures/Basketball.jpg");
	mGradientMap.Initialize(L"../Textures/gradient_map.jpg");

	//Initialize Shaders
	mVShader.Initialize(L"../Data/Shaders/ToonShading.fx");
	mPShader.Initialize(L"../Data/Shaders/ToonShading.fx");

	mSkyboxPixelShader.Initialize(L"../Data/Shaders/CubeMapping.fx");
	mSkyboxVertexShader.Initialize(L"../Data/Shaders/CubeMapping.fx");

	mPostProcessingPixelShader.Initialize(L"../Data/Shaders/Outline.fx");
	mPostProcessingVertexShader.Initialize(L"../Data/Shaders/Outline.fx");
	...
	mPointSampler.Initialize(Sampler::Filter::Point, Sampler::AddressMode::Clamp);

}

void GameApp::OnTerminate()
{
	mPointSampler.Terminate();
	mSampler.Terminate();
