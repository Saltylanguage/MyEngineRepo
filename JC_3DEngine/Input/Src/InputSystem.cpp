...
Ray Input::InputSystem::GetMouseRay(Graphics::Camera& camera)
{
	const int screenX = InputSystem::Get()->GetMouseScreenX();
	const int screenY = InputSystem::Get()->GetMouseScreenY();
	return GetScreenRay(screenX, screenY, camera);
}

Ray Input::InputSystem::GetScreenRay(int screenX, int screenY, Graphics::Camera& camera)
{
	const uint32_t screenWidth = Graphics::GraphicsSystem::Get()->GetWidth(); //TODO:: check architecture with Pchan. Should Input have a reference to Graphics.
	const uint32_t screenHeight = Graphics::GraphicsSystem::Get()->GetHeight();
	return camera.ScreenPointToRay(screenX, screenY, screenWidth, screenHeight); //TODO:: check architecture with Pchan. Where should camera be.
}
