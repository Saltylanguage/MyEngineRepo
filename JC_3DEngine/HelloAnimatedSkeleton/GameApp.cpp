...
void GameApp::OnInitialize(uint32_t width, uint32_t height)
{
	...
	mTerrain.SetLayer(L"../Textures/Grass.jpg", 0, 0.0f, 15.0f, 15.0f); //first value:1st texture  second value: layer third value: minLimit  fourth value: maxLimit last value: tileFrequency
	...
	mVShader.Initialize(L"../Data/Shaders/Skinning.fx");
...
void GameApp::OnUpdate()
{
	...
	for (size_t i = 0; i < boneTransforms.size() && i < 128; i++)
	{
		boneData.matrices[i] = Matrix::Transpose(boneTransforms[i]);
	}

	mBoneConstantBuffer.Set(boneData);
	mBoneConstantBuffer.BindVS(1); 
