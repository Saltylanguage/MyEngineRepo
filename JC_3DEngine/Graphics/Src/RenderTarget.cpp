...
RenderTarget::~RenderTarget()
{
	ASSERT(mShaderResourceView == nullptr, "[RenderTarget] mShaderResourceView was not terminated.");
	ASSERT(mRenderTargetView == nullptr, "[RenderTarget] mRenderTargetView was not terminated.");
	ASSERT(mDepthStencilView == nullptr, "[RenderTarget] mDepthStencilView was not terminated.");
}
...
void RenderTarget::UnbindPS(uint32_t index)
{
	...
	GraphicsSystem::Get()->GetContext()->PSGetShaderResources(index, 1, &dummy);
}
